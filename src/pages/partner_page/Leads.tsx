...
import { Search, Download, Plus, ArrowUpDown, MoreVertical, ChevronLeft, ChevronRight } from 'lucide-react';
...
const Leads = () => {
  ...
  const [leads, setLeads] = useState<Lead[]>([
    ...
    { leadId: 'LD-108', leadName: 'Priya', type: 'Birhday', createdDate: 'March 26 2025', editedDate: 'March 29 2025', status: 'Done' },
  ...
  const handleDownload = () => {
    ...
    exportToExcel(excelData, 'leads', ['Lead ID', 'Lead Name', 'Type', 'Created Date', 'Edited Date', 'Status']);
  };
  ...
  const handleSaveLead = (leadData: Partial<Lead>) => {
    if (modalMode === 'add') {
      ...
      const newLead: Lead = {
        ...
        type: leadData.type || leadData.eventType || '',
        createdDate: leadData.createdDate || new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
        editedDate: leadData.editedDate || new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
      ...
      const updatedLeads = leads.map(lead =>
        lead.leadId === selectedLead.leadId
          ? {
              ...lead,
              ...leadData,
              leadName: leadData.leadName || `${leadData.firstName} ${leadData.lastName}`.trim() || lead.leadName,
              type: leadData.type || leadData.eventType || lead.type,
